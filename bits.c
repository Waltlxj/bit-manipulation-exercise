...
*/
int sign(int x) {
    // !!x returns 0 if 0, 1 otherwise
    // x >> 31 return 11...1 if negative, 0 otherwise
    return (!!x) | (x >> 31);
}
/*
...
int conditional(int x, int y, int z) {
  // ! used to turn integer to boolean, then I tried to use the overflow (after flipping them)
  return ((!x+~0) & y) + ((~!x + 1) & z);
}
/*
...
int logicalNeg(int x) {
  // ~x + 1 will flip zero twice but others once
  return (((~x + 1) | x) >> 31) + 1;
}
/*
...
int isLessOrEqual(int x, int y) {
  // when signs are the same, I dont worry about overflow
  // Simplifying ((~(x^y) >> 31) & 1) & (((x + ~y) >> 31) & 1)
  int diff_sign = ((~(x^y) & (x + ~y)) >> 31) & 1; // x-y = x+ ~y + 1 <= 0, x + ~y < -1 ----- zero handled
  
  // When signs are different, evaluate the truth directly without calculation
  // Simplifying (((x^y) >> 31) & 1) & ((~y >> 31) & 1)
  int same_sign = (((x^y) & (~y)) >> 31) & 1;
 ...
 */
int absVal(int x) {
  // if x < 0 put ~x + 1 else & 0, if x > 0 put x else & 0.
 ...
 */
int isPower2(int x) {
  // x & (x -1) = 0 AND most sig. digit is NOT 1 AND x is NOT zero
  // x - 1 = 2x + ~x
  return !(x & ((x << 1) + (~x))) & !(x >> 31) & (!!x);
}
